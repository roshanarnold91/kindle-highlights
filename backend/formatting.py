...
def _note_callout_html(text, *, standalone):
    ...
    return (
        ...
        'letter-spacing:0.04em;color:#92400e;margin-bottom:3px;">📝 NOTE</div>'
        '<div style="color:#78350f;white-space:pre-wrap;">{text}</div>'
        "</div>"
    ).format(top=0 if standalone else 8, size=label_size, text=_esc(text).replace("\n", "<br>"))
...
def format_highlight_block_html(highlight, pref):
    ...
    if highlight.type == "note":
        inner = (
            ...
            'letter-spacing:0.04em;color:#6b7280;margin-bottom:4px;">📝 NOTE</div>'
    ...
    if highlight.type == "bookmark":
        block = (
            ...
            'font-style:italic;border-left:3px solid #d1d5db;">🔖 Bookmark'
        )
        block += _meta_html(highlight, pref)
        block += "</div>"
        return block
    ...
    block = (
        ...
        '<div style="font-size:11px;font-weight:700;'
        'letter-spacing:0.04em;color:#64748b;margin-bottom:4px;">🖍️ HIGHLIGHT</div>'
